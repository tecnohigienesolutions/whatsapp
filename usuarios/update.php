...
if(
   ...
   // $usuarios->fecha_creado = date('Y-m-d H:i:s'); 
	...
	if($usuarios->update()){     
		...
		echo json_encode(array("message" => "El usuario se actualizó."));
	}else{    
		http_response_code(503);     
		echo json_encode(array("message" => "No se puede actualizar el usuario."));
	}
	
} else {
	http_response_code(400);    
    echo json_encode(array("message" => "No se puede actualizar el usuario. Los datos están incompletos."));
}
